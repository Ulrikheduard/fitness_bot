...
async def handle_video(message: Message):
    ...
    # Если уже отметил задание как выполненное сегодня
    if task_status == "done":
        await message.answer("Ты уже отметил выполнение задания на сегодня! ✅")
        return

    # Если использовал day off на сегодня
    if task_status == "dayoff":
        await message.answer("Ты использовал day off на сегодня. Видео не требуется.")
        return
    ...
    # Отмечаем задание как выполненное с видео и начисляем баллы
    await mark_task_done(user_id, today, video_file_id)
    await update_score(user_id, 2)  # +2 балла за выполнение
    ...
    await message.answer(
        ...
        f"Твой рейтинг: {updated_user['score']} бицепсов."
    )
...
async def done_challenge(callback: CallbackQuery):
    ...
    # Просто просим прислать видео (не отмечаем задание пока)
    updated_user = await get_user(user_id)
